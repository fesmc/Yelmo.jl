...
| Surface velocity reference | Joughin et al. 2018 (J18) | `GRL-16KM_VEL-J18.nc` |
| Drainage basins | NASA / NEGIS | `GRL-16KM_BASINS-nasa.nc` |
...
- Joughin, I. et al. (2018). A Greenland Ice Sheet model evaluation
  based on observed ice velocities. *The Cryosphere*, 12(6), 1829–1847.
...
- The default run is 100 yr (`T_END_YR = 100.0`) as a quick functional
  test. For a proper equilibrium spin-up, increase to 5000–20 000 yr
  and consider using the `equil_method = "opt"` friction optimisation
  from the Fortran reference (not yet ported to Yelmo.jl).
